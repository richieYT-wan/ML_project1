...
def k_split(y, x, k_indices, k):
    ...
    input : y (np.array) : target
            x (np.array) : data set
            k_indices (np.array) : set of indices from build_k_indices
            k (int) : k'th group 
    """
...
def crossval_ridge_gridsearch(y, tx_clust, k_fold, lambdas, degrees, loss=False):
    ...
    Output : w_opt : Optimal weights found during cross-validation
             best_degree (int) : Best degree for feature augmentation to be re-used on the test set
             best lambda (float) : Best lambda for ridge regression
    ...
    print("Iterating. Testing {} lambdas for {} degrees".format(len(lambdas),len(degrees)))
    ...
    print("Ridge regression : getting optimal weights with features augmented to degree ({}), with lambda ({})".format(best_degree,best_lambda))
    
    tx_poly_best = build_poly(tx_clust,best_degree)
    w_opt, _ = ridge_regression(y,tx_poly_best,best_lambda)
    if loss==False:
        print("Cross-validation done, returning optimal weight, degree, lambda.")
        return w_opt, best_degree, best_lambda
    elif loss:
        print("Cross-validation done, returning optimal weight, degree, lambda \nAnd saved train and test loss arrays for visualization")
...
def crossval_regulog_gridsearch(y, tx_clust, k_fold, lambdas, degrees, 
    ...
    Output : w_opt : Optimal weights found during cross-validation
             best_degree (int) : Best degree for feature augmentation to be re-used on the test set
             best lambda (float) : Best lambda for regularized regression
             total_train_loss, total_test_loss (optional) : Arrays of losses for visualization
    """
    #Initializing values. 
    k_indices = build_k_indices(y, k_fold)
