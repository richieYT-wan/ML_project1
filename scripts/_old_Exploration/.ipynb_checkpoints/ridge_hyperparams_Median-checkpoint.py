"""SCRIPT USED FOR FINDING THE BEST HYPER-PARAMS fOR RIDGE REGRESSION

After finding the best way to pre-process the data, this script was ran to test replacing-999 values 
using either median or mean, and which hyper params to be used for Ridge Regression.
"""
...
w_opt0, d_opt0, la0, train0, test0 = crossval_ridge_gridsearch(y0,tx0,k_fold,
                                                             lambdas,degrees,
                                                             loss=True)
cv_viz(d_opt0,lambdas,train0[d_opt0-1,:],test0[d_opt0-1,:],save="ridge_clust0REM0_FINAL_LOGPRO")
#----------------1
print("#======== CV for Cluster 1 ========#")
w_opt1, d_opt1, la1, train1, test1 = crossval_ridge_gridsearch(y1,tx1,k_fold,
                                                             lambdas,degrees,
                                                             loss=True)
cv_viz(d_opt1,lambdas,train1[d_opt1-1,:],test0[d_opt1-1,:],save="ridge_clust1REM0_FINAL_LOGPRO")
...
w_opt2, d_opt2, la2, train2, test2 = crossval_ridge_gridsearch(y2,tx2,k_fold,
                                                             lambdas,degrees,
                                                             loss=True)
cv_viz(d_opt2,lambdas,train2[d_opt2-1,:],test0[d_opt2-1,:],save="ridge_clust2REM0_FINAL_LOGPRO")
...
w_opt3, d_opt3, la3, train3, test3 = crossval_ridge_gridsearch(y3,tx3,k_fold,
                                                             lambdas,degrees,
                                                             loss=True)
cv_viz(d_opt3,lambdas,train3[d_opt3-1,:],test0[d_opt3-1,:],save="ridge_clust3REM0_FINAL_LOGPRO")
...
#Test preprocessing
test0, i0, test1, i1, test2, i2, test3, i3 = cluster_preprocessing_test(tX_test, id0, id1,id2,id3, degs, num2name,f="median")
#Prediction
...
create_csv_submission(ids_test, yclusterpred_opt, OUTPUT_PATH+"RIDGE_WITHLOGPROCESS_FINAL.csv")
